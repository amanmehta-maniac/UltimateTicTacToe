...
class ticTacToe:
	...
	def check_status(self,board,old_move,player,block):
		...
		for i in range(4):
			if board[x+i][y].count('x') + board[x+i][y+1].count('x') + board[x+i][y+2].count('x') + board[x+i][y+3].count('x') == 3 and board[x+i][y].count('-') + board[x+i][y+1].count('-') + board[x+i][y+2].count('-') + board[x+i][y+3].count('-') == 1:
				if player == 'x': return 5
				else: 
					if old_move[0] == x+i and old_move[1] in [y,y+1,y+2,y+3]: return 150
					return -150 

			if board[x+i][y].count('o') + board[x+i][y+1].count('o') + board[x+i][y+2].count('o') + board[x+i][y+3].count('o') == 3 and board[x+i][y].count('-') + board[x+i][y+1].count('-') + board[x+i][y+2].count('-') + board[x+i][y+3].count('-') == 1:
				if player == 'o': return 5
				else: 
					if old_move[0] == x+i and old_move[1] in [y,y+1,y+2,y+3]: return 150
					return -150 

			if board[x][y+i].count('x') + board[x+1][y+i].count('x') + board[x+2][y+i].count('x') + board[x+3][y+i].count('x') == 3 and board[x][y+i].count('-') + board[x+1][y+i].count('-') + board[x+2][y+i].count('-') + board[x+3][y+i].count('-') == 1:
				if player == 'x': return 5
				else:
					if old_move[0] in [x,x+1,x+2,x+3] and old_move[1] == y+i: return 150
					return -150

			if board[x][y+i].count('o') + board[x+1][y+i].count('o') + board[x+2][y+i].count('o') + board[x+3][y+i].count('o') == 3 and board[x][y+i].count('-') + board[x+1][y+i].count('-') + board[x+2][y+i].count('-') + board[x+3][y+i].count('-') == 1:
				if player == 'o': return 5
				else:
					if old_move[0] in [x,x+1,x+2,x+3] and old_move[1] == y+i: return 150
					return -150  
		...
		if board[x][y].count('x') + board[x+1][y+1].count('x') + board[x+2][y+2].count('x') + board[x+3][y+3].count('x') == 3 and board[x][y].count('-') + board[x+1][y+1].count('-') + board[x+2][y+2].count('-') + board[x+3][y+3].count('-') == 1:
			if player == 'x': return 5
			else:
				if old_move[0] - x == old_move[1] - y: return 150 
				return -150 				
		
		if board[x][y].count('o') + board[x+1][y+1].count('o') + board[x+2][y+2].count('o') + board[x+3][y+3].count('o') == 3 and board[x][y].count('-') + board[x+1][y+1].count('-') + board[x+2][y+2].count('-') + board[x+3][y+3].count('-') == 1:
			if player == 'o': return 5
			else:
				if old_move[0] - x == old_move[1] - y: return 150 
				return -150 				
					
		if board[x][y+3].count('x') + board[x+1][y+2].count('x') + board[x+2][y+1].count('x') + board[x+3][y].count('x') == 3 and board[x][y+3].count('-') + board[x+1][y+2].count('-') + board[x+2][y+1].count('-') + board[x+3][y].count('-') == 1:
			if player == 'x': return 5
			else:
				if old_move[0] + old_move[1] == 3 + x + y: return 150 
				return -150 				

		if board[x][y+3].count('o') + board[x+1][y+2].count('o') + board[x+2][y+1].count('o') + board[x+3][y].count('o') == 3 and board[x][y+3].count('-') + board[x+1][y+2].count('-') + board[x+2][y+1].count('-') + board[x+3][y].count('-') == 1:
			if player == 'o': return 5
			else:
				if old_move[0] + old_move[1] == 3 + x + y: return 150 
				return -150 				
		...
		for i in range(4):
			if board[x+i][y] == board[x+i][y+1] == board[x+i][y+2] == board[x+i][y+3]:
				if player == board[x+i][y]: 
					if old_move[0] == x+i and old_move[1] in [y,y+1,y+2,y+3]: return 110
				elif board[x+i][y] != '-' and board[x+i][y] != player: 
					return -110 
			if board[x][y+i] == board[x+1][y+i] == board[x+2][y+i] == board[x+3][y+i]:
				if player == board[x][y+i]: 
					if old_move[0] in [x,x+1,x+2,x+3] and old_move[1] == y+i: return 110
				elif board[x][y+i] != '-' and board[x][y+i] != player: 
					return -110  
					

		if board[x][y] == board[x+1][y+1] ==  board[x+2][y+2] == board[x+3][y+3]:
			if player == board[x][y]: 
				if old_move[0] - x == old_move[1] - y :return 110
			elif board[x][y] != '-' and board[x][y] != player :
				return -110 
			
		if board[x][y+3] ==  board[x+1][y+2] == board[x+2][y+1] == board[x+3][y]:
			if player == board[x][y+3]: 
				if old_move[0] + old_move[1] == 3 + x + y: return 110
			elif board[x][y+3] != '-' and board[x][y+3] != player:
				return -110
