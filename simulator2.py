...
class tictac():
	...
	def move(self, board, old_move, flag):
		...
		#Find the list of valid cells allowed
		cells = ticTacToe2().find_valid_move_cells(board,old_move,flag)
		return cells
			...
			for move in allowed_moves:
				if level >= allowed_level:
					...
					if self.type == 'x':
						...
						self.type = 'x'
					#print 'retrning ', our, 'them: ', them
					return (r, c, our - them)

				self.board.update(old_move, move, self.type)

				allowed_moves = self.board.find_valid_move_cells(move)
